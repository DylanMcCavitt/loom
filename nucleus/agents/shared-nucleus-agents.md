...
Activation machinery (plugin-bridge rendering, shared-agent package/eval validators) was retired by the skill-pack pivot (LOO-220): packages load directly from the canonical repo tree, and any live-HOME write still requires explicit HITL approval. Superseded `omp-*` candidates and historical issue context live in the JSON contract.
