...
function makeFixture() {
  ...
  writeFileSync(path.join(root, "package.json"), JSON.stringify({ name: "loom", scripts: { "render-nucleus": "node scripts/render-nucleus.mjs", "install-nucleus": "node scripts/render-nucleus.mjs --write", check: "npm run validate", doctor: "node scripts/doctor.mjs" } }, null, 2));
