...
- **Evals on cloud VMs:** run from the **loom repo root**. `.cursor/environment.json` installs `agent` and `codex` CLIs on boot; set `LOOM_JUDGE_BACKEND` (`cursor` or `codex`) and `CURSOR_API_KEY` in Cloud Agents Secrets, then `source .cursor/source-eval-judge.sh` before `npm run bench -- --judge`. Full cadence: [`docs/operator/evals.md`](docs/operator/evals.md).
